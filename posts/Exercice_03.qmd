...
On va ainsi supposer avoir observé les deux vecteurs $x$ et $y$ précédents, sans connaître le lien théorique précédent qui lie $x$ et $y$. Et on cherchera donc à l'estimer.


# Ananlyse descriptive
...
::: success
On voit bien que notre variable $x$ a une distribution normale centré réduite et que $y$, de manière logique avec le modèle simulé, a d'avantages de valeurs négatives.
:::
...
::: success
On voit que la corrélation est faible. Pourtant le nuage de point semble quand à lui témoigner d'une influence de $x$ sur $y$

Puis on retrouve un belle histogramme de distibution $\mathcal{N}(0, 1)$ pour notre variable $x$.
:::
...
A partir du *summary()* de ces différents modèles et de différentes fonctions *R*, on est capable ensuite capable d'obtenir différents critères de sélection.
...
Ainsi, on peut voir que c'est *mod2* qui ressort comme étant le meilleur modèle avec de forte valeur de $R^2$ et $R^2_{adjusted}$ puis des critères AIC et BIC minimisé.
...
## Rappels validation croisée 

texte avec dedans le principe du LOO !!!!

## Validation "manuelle"

Dans un premier temps et pour bien comprendre la méthode, on va utiliser deux fonctions permettant d’estimer l’erreur test par une validation
croisée *LOO* (*Leave-one-out*) pour un modèle ajusté par la fonction *lm* :

- la première, *loo* (qui se trouve dans les fonctions du setup), en utilisant le principe général de cette méthode qui nécessite donc l’estimation de n modèles différents 


- a seconde, *loo2* (qui se trouve dans les fonctions du setup), en utilisant la formule adaptée à la régression linéaire donnant directement le risque LOO à partir de la seule estimation du modèle complet (on pourra utiliser la fonction *hatvalues*)
...
results <- data.frame(
  Model = model_names,
  LOO = unlist(lapply(loo_results, function(x) x$result)),       # Résultat de loo()
  Time_LOO = unlist(lapply(loo_results, function(x) x$time)),    # Temps d'exécution de loo()
  LOO2 = unlist(lapply(loo2_results, function(x) x$result)),     # Résultat de loo2()
  Time_LOO2 = unlist(lapply(loo2_results, function(x) x$time))   # Temps d'exécution de loo2()
)
...
Et en terme de qualité de modèle, c'est bien mod2 qui minimise le *loo*
:::
...
La fonction cv.glm de la librairie boot permet d’estimer l’erreur test par validation croisée K-fold


On va donc maintenant utiliser la fonction *cv.glm* de la library \{ boot \}. Cela va nécessité de recalculer le modèle mais cette fois ci avec la fonction *glm* en spécifiant que l'on veut un modèle gaussien (ce qui nous donnera le même résultat qu'avec *lm*).
...
Et en terme de qualité de modèle, c'est bien mod2 qui minimise la *cv*.
:::
...
D'après tout ce que l'on a pu voir durant cette étude, jusqu'à présent le meilleut modèle semble être mod2.
```{r}
mod2 %>% summary()
```
...
Donc à la vue de ces résulats je pense que l'on peut dire sans prendre trop de risques que notre estimation est bonne.
:::


# Conclusion

On voit qu'on a bien réussi à retrouver le lien théorique via le test d pluseurs modèle et l'utilisation de plusieurqs critères couplé à de la validation croisée pour affiner notre recherche du modèle le mieux ajusté.
