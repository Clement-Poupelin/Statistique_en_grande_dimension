...
index_draw <- sample(nrow(Hitters_Without_NA), size = floor(percent_to_draw * nrow(Hitters_Without_NA)))

Hitters_train <- Hitters_Without_NA[indices_train, ]
dim(Hitters_train)

Hitters_test <- Hitters_Without_NA[-indices_train, ]
dim(Hitters_test)
```
...
rmsep.cv_mod_pcr <- min(RMSEP(mod_pcr, "CV")$val[,,])
rmsep.cv_mod_pcr # 348.3224
nb.com_rmsep.cv_mod_pcr = which.min(RMSEP(mod_pcr, "CV")$val[,,])
nb.com_rmsep.cv_mod_pcr # 14 comps (sans compter l'intercept)
```
...
```{r}
salary.pls <- plsr(Salary ~., scale = TRUE, data = Hitters_train, validation = "CV", segments=10)
summary(salary.pls)
...
hat_Hitters_test_mod_pcr = predict(mod_pcr, Hitters_test)

rmsep_mod_pcr = sqrt(mean((hat_Hitters_test_mod_pcr - Hitters_test$Salary)**2))
rmsep_mod_pcr
# 346.1988


hat_df_test_salary.pls = predict(salary.pls, Hitters_test)

rmsep_salary.pls = sqrt(mean((hat_df_test_salary.pls - Hitters_test$Salary)**2))
rmsep_salary.pls
# 356.0666
